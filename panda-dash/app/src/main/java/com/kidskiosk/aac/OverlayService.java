...
public class OverlayService extends Service {
    ...
    public class OverlayBridge {
        ...
        public void showContentView() {
            // Restore content view when returning from a native app
            if (contentView != null) {
                contentView.post(() -> contentView.setVisibility(android.view.View.VISIBLE));
            }
        }
    ...
    private void launchApp(String packageName) {
        // Shrink overlay to just the tab so app launches fully visible
        if (overlayParams != null && windowManager != null) {
            overlayParams.width = dpToPx(44); // just the tab width
            try { windowManager.updateViewLayout(kioskView, overlayParams); } catch (Exception e) {}
        }
        // Hide content view so native app shows through
        if (contentView != null) {
            contentView.post(() -> contentView.setVisibility(android.view.View.GONE));
        }
        PackageManager pm = getPackageManager();
